...
var _ = ginkgo.Describe("[sig-node] RuntimeClass", func() {
	...
	ginkgo.It("should reject a Pod requesting a RuntimeClass with conflict node selector", func() {
		handler := framework.GetRuntimeClassHandler("test-handler")
		...
		runtimeClass := newRuntimeClass(f.Namespace.Name, "conflict-runtimeclass", handler)
	...
	ginkgo.It("should run a Pod requesting a RuntimeClass with scheduling [NodeFeature:RuntimeHandler] ", func() {
		...
		runtimeClass := newRuntimeClass(f.Namespace.Name, "non-conflict-runtimeclass", handler)
		runtimeClass.Scheduling = scheduling
		rc, err := f.ClientSet.NodeV1beta1().RuntimeClasses().Create(runtimeClass)
		framework.ExpectNoError(err, "failed to create RuntimeClass resource")
		...
		labelPod, err := f.ClientSet.CoreV1().Pods(f.Namespace.Name).Get(pod.Name, metav1.GetOptions{})
...
func newRuntimeClass(namespace, name, handler string) *v1beta1.RuntimeClass {
	uniqueName := fmt.Sprintf("%s-%s", namespace, name)
	return runtimeclasstest.NewRuntimeClass(uniqueName, handler)
}
