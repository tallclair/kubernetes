...
all_packages=()
while IFS='' read -r line; do all_packages+=("$line"); done < <(go list -e "${FOCUS:-.}"/... | grep -vE "/(third_party|vendor|staging/src/k8s.io/client-go/pkg|generated|clientset_generated)" | sed -e 's|/staging/src/|/vendor/|' -e 's|^k8s.io/kubernetes/|./|' -e "s|^_\(${KUBE_ROOT}/\)\{0,1\}||")
failing_packages=()
