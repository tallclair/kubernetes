...
func logPostTimeoutResult(ctx context.Context, timedOutAt time.Time, r *result) {
	if r == nil {
		...
		klog.ErrorS(nil, "FinishRequest: post-timeout activity, child goroutine has not returned yet",
			"timeElapsed", time.Since(timedOutAt),
			audit.AuditIDLogKey, audit.GetAuditIDTruncated(ctx))
		return
	}
	...
	err := fmt.Errorf("FinishRequest: post-timeout activity - time-elapsed: %s, %s: %s, panicked: %t, err: %v, panic-reason: %v",
		time.Since(timedOutAt), audit.AuditIDLogKey, audit.GetAuditIDTruncated(ctx), r.reason != nil, r.err, r.reason)
