...
func newWorker(
	...
	switch probeType {
	case prober.Readiness:
		w.spec = container.ReadinessProbe
		w.resultsManager = m.readinessManager
		w.initialValue = results.Failure
	case prober.Liveness:
...
func (w *worker) run(ctx context.Context) {
	...
	defer func() {
		...
		prober.ProberResults.Delete(w.proberResultsSuccessfulMetricLabels)
		prober.ProberResults.Delete(w.proberResultsFailedMetricLabels)
		prober.ProberResults.Delete(w.proberResultsUnknownMetricLabels)
		prober.ProberDuration.Delete(w.proberDurationSuccessfulMetricLabels)
		prober.ProberDuration.Delete(w.proberDurationUnknownMetricLabels)
	}()
...
func (w *worker) doProbe(ctx context.Context) (keepGoing bool) {
	...
	if w.containerID.String() != c.ContainerID {
		...
		if !utilfeature.DefaultFeatureGate.Enabled(features.ChangeContainerStatusOnKubeletRestart) {
			...
			if isRestartableInitContainer && w.probeType == prober.Startup {
	...
	if w.pod.ObjectMeta.DeletionTimestamp != nil && (w.probeType == prober.Liveness || w.probeType == prober.Startup) {
	...
	case results.Success:
		prober.ProberResults.With(w.proberResultsSuccessfulMetricLabels).Inc()
		prober.ProberDuration.With(w.proberDurationSuccessfulMetricLabels).Observe(time.Since(startTime).Seconds())
	case results.Failure:
		prober.ProberResults.With(w.proberResultsFailedMetricLabels).Inc()
	default:
		prober.ProberResults.With(w.proberResultsUnknownMetricLabels).Inc()
		prober.ProberDuration.With(w.proberDurationUnknownMetricLabels).Observe(time.Since(startTime).Seconds())
	}
	...
	if (w.probeType == prober.Liveness && result == results.Failure) || w.probeType == prober.Startup {
