...
func (p *cadvisorStatsProvider) PodCPUAndMemoryStats(ctx context.Context, pod *v1.Pod, _ *kubecontainer.PodStatus) (*statsapi.PodStats, error) {
	pcm := p.containerManager.NewPodContainerManager()
	_, podName := pcm.GetPodContainerName(pod)
	// podName := cm.GetPodCgroupNameSuffix(pod.UID)
	klog.InfoS(">>>> Start PodCPUAndMemoryStats", "pod", klog.KObj(pod), "podName", podName) // FIXME
	...
	klog.InfoS(">>>> Found pod stats!", "pod", klog.KObj(pod), "infos", infos) // FIXME
	...
	if _, ok := infos[podName]; !ok { // FIXME
		klog.ErrorS(nil, ">>>> MISSING pod info!", "pod", klog.KObj(pod), "podName", podName)
	}
	...
	klog.InfoS(">>>> PodCPUAndMemoryStats", "pod", klog.KObj(pod), "stats", *podStats)
