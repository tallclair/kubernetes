...
func (p *criStatsProvider) PodCPUAndMemoryStats(ctx context.Context, pod *v1.Pod, podStatus *kubecontainer.PodStatus) (*statsapi.PodStats, error) {
	...
	// Fallback if ther ListContainerStats doesn't return any results.
	...
	if useFallback {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to fetch stats from fallback provider: %w", err)
		}
		if ps.CPU == nil {
...
func (p *criStatsProvider) makeContainerCPUAndMemoryStats(
	...
	if stats.Cpu != nil {
		result.CPU = &statsapi.CPUStats{
			...
			UsageNanoCores:       getUint64(stats.Cpu.UsageNanoCores),
			UsageCoreNanoSeconds: p.getContainerUsageNanoCores(stats),
